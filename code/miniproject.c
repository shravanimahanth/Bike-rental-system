...
struct CustomerDetails  //Creating structure to store customer details
{
	char bikename[30];
    char username[30];
	char password[30];
...
int main()
{
	system("color 70");  // creating background colour
	WelcomePage();
}

void Title()    //creating title
{
	printf("\n\t\t\t\t\t");
	for (int i = 0; i < 81; i++)
		printf("-");
...
void WelcomePage() //creating welocomepage
{
	time_t t;
	time(&t);
	int i = 0, choice;
...
void registerMenu()    //function for registration
{
	system("cls");
	char user_name[30], password[20], UName[20], Pass[20];
	int valid=0;
	FILE *usp, *read;
	usp = fopen("Register.txt", "a+");  //appending the file
	read = fopen("Register.txt", "r");   //read from the file
	printf("\n\n\n\t\t\t-------------------------------------------------------  SIGNUP  -------------------------------------------------------\n");
	A:
		printf("\n\t\t\t\t\t\tENTER USERNAME : ");
		gets(user_name);
...
void login()   //function to login
{
	...
	{
		ch = getch();
		if (ch != 13 && ch != 8)  //hiding the password
		{
			printf("*");
			pw[i] = ch;
			i++;
	...
	{
		if (strcmp(s.username, un) == 0)   // checks for correct username
		{
			flag = 1;
			if (strcmp(s.password, pw) == 0)  //checks for correct password
			{
				printf("\n\n\n\t\t\t\t\t\t\t      ---------- LOGIN SUCCESSFUL ----------");
				getch();
				fclose(fp);
...
void UserMenu(char username[30])  //function to display the userpage
{
	int i = 0;
	...
	while (1)
	{
		...
		for (i = 0; i < 80; i++)
			printf(">");
		printf("\n\t\t\t\t\tCurrent date and time : %s", ctime(&t));  //displays the register time
		printf("\t\t\t\t\t");
		for (i = 0; i < 80; i++)
			printf("<");
		scanf("%d", &choice);
...
void adminMenu()  //function to display the admin page
{
	int choice, i = 0;
	time_t t;
	time(&t);
...
void RentBike(char username[30])   //function to rent a bike
{
	FILE *f,*fp,*fp1,*fp2;
	time_t t;
	time(&t);
...
bikename:
	...
	{
		if (strcmp(tempBike, s.bikename) == 0)  // checks if the bike name matches with the list of bikes in file
		{
			flag = 1;
		}
	}
	...
	while (fscanf(fp, "%s %s %s\n", vehicle, price, stock) != EOF)
	{
		if (strcmp(vehicle, s.bikename) == 0)
		{
			...
			{
				flag = 1;
				stockLeft--;  // changes the stock of bikes after every purchase
...
void listOfRentals()  //function to view the rental record of all the users from admin side
{
	FILE *f,*fp;
	int i, q = 9;
	...
	system("cls");
		Title();
	gotoxy(0,5);    //for correct spacing
	for (i = 0; i < 155; i++)
	{
		printf("-");
	}
...
void UserRentedList(char username[30])  //function to view the rental records
{
...
void delete()  // function to delete the user
{
	FILE *f, *t;
	int flag = 0;
	...
	else
	{
		...
		while (fscanf(f, "%s %s %s %s %s %s %s %s\n", s.username,s.password,s.fullname,s.address,s.phonenumber,s.nationality,s.email,s.license) != EOF)
		{
			if (strcmp(custUserName, s.username) == 0)  // checks if user name matches with the registered user
			{
                flag = 1;
			}
			else
...
void ViewFares()  // function to display fares and stock of the vehicle
{
	system("cls");
	FILE *fp;
	char vehicle[20];
...
void EditFares()  //function to update the user details
{
	system("cls");
	Title();
	FILE *fp, *r;
...
void search()  // function to search a user from admin side
{
	system("cls");
	FILE *f,*fp;
...
void UserSearch(char username[20])  //function to search the rented record fron user side
{
    system("cls");
	FILE *f,*fp;
	...
	else
	{
		...
		while(fscanf(fp,"%s %s %i %s\n",s.bikename,user_name,&s.period,time) != EOF)
		{
			if (strcmp(bikename, s.bikename) == 0)
			{
				...
				while (fscanf(f, "%s %s %s %s %s %s %s %s\n", s.username,s.password,s.fullname,s.address,s.phonenumber,s.nationality,s.email,s.license) != EOF)
				{
					if(strcmp(username,s.username)==0 && strcmp(s.username,user_name)==0)  //checks if bike name is in the list of the logged-in user record
					{
						flag = 0;
						printf("\n\n\t\t\t\t\t\t\t=======================================================");
						printf("\n\n\t\t\t\t\t\t\t\t\tRECORD FOUND\n");
...
void edit()  //function to edit the details from the admin side
{
	FILE *f, *r;
	int k = 0,t = 0;
	char ed;
	...
	else
	{
		...
		while (fscanf(f, "%s %s %s %s %s %s %s %s\n", s.username,s.password,s.fullname,s.address,s.phonenumber,s.nationality,s.email,s.license) != EOF)
		{
            if(strcmp(username,s.username) == 0)  // verifies with the username
            {
                k = 2;
                if (strcmp(name, s.fullname) == 0) // verifies with the full name and proceeds
                {
                    t = 1;
                    printf("\n\t\t\t\t\t\t\t\t\t*** Existing Record ***");
                    printf("\n\t\t\t\t\t\t\t\tName\t\t: %s", s.fullname);
        ...
        else if((k == 2) && (t != 1))  //if username and full name won't match with the given details
        {
...
void generateBill(char username[20], char bikename[20], int period) //generates the bill after renting a bike
{
	time_t t;
...
void loginForAdmin() //function to login as admin
{
	system("cls"); // clears the screen
	char username[] = "bikerentalsystem";  //hard-coding the username
	char pw[] = "miniproject";  //hard-coding the password
	...
	if (strcmp(username, UName) == 0) //verifies the entered username with the hard-corded username
	{
		flag = 1;
		if (strcmp(password, pw) == 0) //verifies the entered password with the hard-corded password
		{
			printf("\n\n\n\t\t\t\t\t\t\t      ---------- LOGIN SUCCESSFUL ----------");
			getch();
			adminMenu();
...
void gotoxy(int x, int y) //transfering control to respective coordinates
{
	COORD CR;
	CR.X = x;
	CR.Y = y;
